...
def bootstrap(model_path: str, 
    ...
    ref_ds.push_to_hub(save_ds, 
                       revision=f'bootstrap_{bootstrap_id}', 
                       commit_message=f"Bootstrap {bootstrap_id} from {ref_ds_name}")

    # Autoformalize the sampled dataset
    infer_hf_dataset(model_path=model_path,
                     dataset_id=save_ds,
                     output_dataset_id=save_ds,
                     dataset_branch=f'bootstrap_{bootstrap_id}',
                     output_dataset_branch=f'bootstrap_{bootstrap_id}',
                     n_samples=1,
                     temperature=temperature,
                     max_tokens=max_tokens)
    
    # Get Lean and GPT feedback
    all_feedback(input_dataset_id=save_ds,
                 input_dataset_branch=f'bootstrap_{bootstrap_id}',
                 output_dataset_id=save_ds,
                 output_dataset_branch=f'bootstrap_{bootstrap_id}',
